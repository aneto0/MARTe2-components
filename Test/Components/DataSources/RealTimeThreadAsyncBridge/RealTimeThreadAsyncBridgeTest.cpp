...
const char8 *RealTimeThreadAsyncBridgeTestDS::GetBrokerName(
		StructuredDataI &data, const SignalDirection direction) {
...
void RealTimeThreadAsyncBridgeTestInputBroker::SetSignalPostMiddle(
		uint32 signalIdx) {
	signalPostMiddle = signalIdx;
}

bool RealTimeThreadAsyncBridgeTestInputBroker::Init(
		const SignalDirection direction, DataSourceI &dataSourceIn,
		const char8 * const functionName, void * const gamMemoryAddress) {
	bool ret = MemoryMapMultiBufferBroker::Init(direction, dataSourceIn,
...
bool RealTimeThreadAsyncBridgeTestInputBroker::Execute() {
	...
	if (copyTable != NULL_PTR(MemoryMapBrokerCopyTableEntry *)) {
		for (n = 0u; (n < numberOfCopies) && (ret); n++) {
			...
			bool ok = dataSource->GetInputOffset(signalIdxArr[n], samples[n],
					ioffset);
			if (ok) {
				...
				if (!offsetChanged) {
					...
					if (allowedSize > (maxOffset[n] - (copyOffsetN + offset))) {
						...
						uint32 numberOfFullCopies = ((allowedSize - overSize)
								/ maxOffset[n]);
						bool isMultiple = (((allowedSize - overSize)
								% maxOffset[n]) == 0)
								&& (allowedSize > overSize);
						...
						if (overSize
								> (maxOffset[n] - (copyOffsetN + offset))) {
						...
						allowedSize = allowedSize
								- (maxOffset[n] * numberOfFullCopies)
								- allowedSizePhase;
					}
				}
				(void) MemoryOperationsHelper::Copy(
				...
				ret = dataSource->TerminateInputCopy(signalIdxArr[n], ioffset,
						samples[n]);
				myOffset[n] = offset;
			} else {
...
void RealTimeThreadAsyncBridgeTestOutputBroker::SetSignalPostMiddle(
		uint32 signalIdx) {
	signalPostMiddle = signalIdx;
}

bool RealTimeThreadAsyncBridgeTestOutputBroker::Init(
		const SignalDirection direction, DataSourceI &dataSourceIn,
		const char8 * const functionName, void * const gamMemoryAddress) {
	bool ret = MemoryMapMultiBufferBroker::Init(direction, dataSourceIn,
			functionName, gamMemoryAddress);
	myOffset = new int32[numberOfCopies];
	goSemWrite =
			((RealTimeThreadAsyncBridgeTestDS*) dataSource)->GetEventWrite();
	return ret;
}
...
bool RealTimeThreadAsyncBridgeTestOutputBroker::Execute() {
	...
	if (copyTable != NULL_PTR(MemoryMapBrokerCopyTableEntry *)) {
		for (n = 0u; (n < numberOfCopies) && (ret); n++) {
			...
			/*lint -e{613} null pointer checked before.*/
			bool ok = dataSource->GetOutputOffset(signalIdxArr[n], samples[n],
					ioffset);
			if (ok) {
				uint32 dataSourceIndex = ((currentBuffer * numberOfCopies) + n);
				uint32 allowedSize = copyTable[n].copySize;
				...
				/*lint -e{613} null pointer checked before.*/
				ret = dataSource->TerminateOutputCopy(signalIdxArr[n], ioffset,
						samples[n]);
				myOffset[n] = offset;
			} else {
				myOffset[n] = 0xFFFFFFFF;
...
static bool InitialiseMemoryMapInputBrokerEnviroment(
...
bool RealTimeThreadAsyncBridgeTest::TestSetConfiguredDatabase() {
	...
	if (ret) {
		dataSource = ObjectRegistryDatabase::Instance()->Find(
	...
	if (ret) {
		...
		uint32 nElements = dataSource->GetNumberOfSignals()
				* dataSource->GetNumberOfMemoryBuffers();
...
bool RealTimeThreadAsyncBridgeTest::TestGetInputOffset() {
	...
	if (ret) {
		gamWriter = ObjectRegistryDatabase::Instance()->Find(
				"Application1.Functions.GAMA");
		ret = gamWriter.IsValid();

	}
	if (ret) {
		gamReader1 = ObjectRegistryDatabase::Instance()->Find(
				"Application1.Functions.GAMB");
	...
	if (ret) {
		...
		/*
		 ...
		 uint32 nElements = nSignals * dataSource->GetNumberOfMemoryBuffers();*/

		//uint32 *gamReader1mem = (uint32 *) gamReader1->GetInputMemoryBuffer();
		...
		for (uint32 cnt = 0u; (cnt < 10 * sizeof(uint32)) && (ret); cnt +=
				sizeof(uint32)) {
			//Sleep::MSec(100);
			//write
			...
			//int32 offset1 = 0; //dataSource->GetInputOffset(1, 0);
		}
...
bool RealTimeThreadAsyncBridgeTest::TestGetOutputOffset() {
	...
	if (ret) {
		...
		ReferenceT < RealTimeThreadAsyncBridgeTestOutputBroker > broker =
				outputBrokers.Get(0);
		ret = broker.IsValid();
		Sleep::MSec(100);
		//goRead->Reset();
...
bool RealTimeThreadAsyncBridgeTest::TestGetInputOffset_2Readers() {
	...
	if (ret) {
		gamReader2 = ObjectRegistryDatabase::Instance()->Find(
				"Application1.Functions.GAMC");
	...
	if (ret) {
		...
		for (uint32 cnt = 0u; (cnt < 10 * sizeof(uint32)) && (ret); cnt +=
			...
			ret = ((cnt % (nBuffers * sizeof(uint32))) == (uint32) x)
					|| (((cnt - sizeof(uint32)) % (nBuffers * sizeof(uint32)))
							== (uint32) x);

			ret &= ((cnt % (nBuffers * sizeof(uint32))) == (uint32) x2)
					|| (((cnt - sizeof(uint32)) % (nBuffers * sizeof(uint32)))
							== (uint32) x2);
			//int32 offset1 = 0; //dataSource->GetInputOffset(1, 0);
		}
...
bool RealTimeThreadAsyncBridgeTest::TestGetInputOffset_Ranges() {
	...
	if (ret) {
		...
		for (uint32 cnt = 0u; (cnt < 100 * sizeof(uint32)) && (ret);
				cnt += 10 * sizeof(uint32)) {
			//Sleep::MSec(100);
			//write
