...
public:
	...
	bool Initialise(MARTe::StructuredDataI & data) {
		...
		if (ok) {
			...
			if (!ok) {
				REPORT_ERROR(ErrorManagement::ParametersError,
						"ExpectedPatterns is not a Matrix");
			}
		}
		if (ok) {
		...
		if (ok) {
			...
			if (!ok) {
				REPORT_ERROR(ErrorManagement::ParametersError,
						"No columns defined in the matrix");
			}
		}
		if (ok) {
			...
			Matrix < uint8
					> expectedPatternsMatrix(expectedPatterns,
							numberOfExpectedPatterns, patternSize);
			ok = data.Read("ExpectedPatterns", expectedPatternsMatrix);
			if (ok) {
				REPORT_ERROR(ErrorManagement::Information,
						"expectedPatternsMatrix set to %d",
						expectedPatternsMatrix);
			} else {
				REPORT_ERROR(ErrorManagement::ParametersError,
						"Could not read the ExpectedPatterns");
			}
		}
	...
	bool Execute() {
		MARTe::uint8 *validSignal =
				reinterpret_cast<MARTe::uint8 *>(GetInputSignalMemory(0));
		MARTe::uint8 *dataSignal =
				reinterpret_cast<MARTe::uint8 *>(GetInputSignalMemory(1));
		if (*validSignal) {
			if (currentPattern < numberOfExpectedPatterns) {
				...
				if (ok) {
					MARTe::Vector < MARTe::uint8
							> vecTemp(dataSignal, patternSize);
					REPORT_ERROR(MARTe::ErrorManagement::Information,
							"Read %d %d ...", *validSignal, vecTemp);
					currentPattern++;
				}
			}
		}
...
bool UARTDataSourceTest::TestInitialise_False_TcnDevice() {
	using namespace MARTe;
	UARTDataSource ds;
	ConfigurationDatabase cdb;
	cdb.Write("PortName", "/dev/pts/1");
...
bool UARTDataSourceTest::TestSetConfiguredDatabase_False_2_Signals() {
	MARTe::TimeoutType timeout(2000);
