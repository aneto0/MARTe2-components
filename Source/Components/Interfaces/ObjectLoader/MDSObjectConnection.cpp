    ...
    if (ret.ErrorsCleared()) {
        ...
        for (uint8 dimIdx = 0u; dimIdx < MDSNumOfDims; dimIdx++) {
            if ( (orientation == "RowMajor") && (dimIdx <= 2u) ) {
                anyTypeParam->SetNumberOfElements(static_cast<uint32>(dimIdx), MDSDimArray[(static_cast<uint32>(MDSNumOfDims) - static_cast<uint32>(dimIdx)) - static_cast<uint32>(1u)]);
            }
            else {
