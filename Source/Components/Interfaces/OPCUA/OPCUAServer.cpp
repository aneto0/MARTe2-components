...
bool OPCUAServer::Initialise(StructuredDataI &data) {
    ...
    if (ok) {
        ...
        if (authenticate) {
            ...
            UA_StatusCode retval = UA_AccessControl_default(config, false, &config->securityPolicies[config->securityPoliciesSize - 1].policyUri, nOfAuthKeys,
                                                            authKeys);
