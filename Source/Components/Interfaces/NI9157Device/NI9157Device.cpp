...
bool NI9157Device::Initialise(StructuredDataI & data) {
    ...
    if (ret) {
        ...
        if (!ret) {
            REPORT_ERROR(ErrorManagement::InitialisationError, "Please spacify the NI-RIO Device name (NiRioDeviceName)");
        }
        if (ret) {
        ...
        if (ret) {
            if (data.Read("Open", isOpened)) {
                if (isOpened == 1u) {
                    REPORT_ERROR_PARAMETERS(ErrorManagement::Information, "Opening %s %s %s", niRioDeviceName.Buffer(), niRioGenSignature.Buffer(), niRioGenFile.Buffer());
    ...
    if (ret) {
        if (isOpened > 0u) {
            if (data.MoveRelative("Configuration")) {
                ...
                NiFpgaEx_ResourceType resTypes[] = { NiFpgaEx_ResourceType_ControlBool, NiFpgaEx_ResourceType_ControlU8, NiFpgaEx_ResourceType_ControlI8, NiFpgaEx_ResourceType_ControlU16, NiFpgaEx_ResourceType_ControlI16,
                        NiFpgaEx_ResourceType_ControlU32, NiFpgaEx_ResourceType_ControlI32, NiFpgaEx_ResourceType_ControlU64, NiFpgaEx_ResourceType_ControlI64 };
                    ...
                    while ((suffixes[i] != NULL_PTR(const char8 *)) && (!found) && (ret)) {
                        ...
                        if (found) {
                            ...
                            if (ret) {
                                ...
                                if (ret) {
                                    ...
                                    if (ret) {
                                        ...
                                        if (ret) {
                                            ...
                                            ret = (status == 0);
                                            if (!ret) {
                                                REPORT_ERROR_PARAMETERS(ErrorManagement::InitialisationError, "Failed writing %s=%d status=%d", variable.Buffer(), value, static_cast<int32>(status));
                                            }
                                            else {
                                                REPORT_ERROR_PARAMETERS(ErrorManagement::Information, "%s=%d status=%d", variable.Buffer(), value, static_cast<int32>(status));
                                            }
                                        }
                                    }
                                    else {
                                        REPORT_ERROR_PARAMETERS(ErrorManagement::InitialisationError, "NI variable %s not found status=%d", variable.Buffer(), status);
                                    }
                                }
                                else {
                                ...
                                REPORT_ERROR(ErrorManagement::InitialisationError, "Failed the creation of CreateNI9157DeviceOperatorI from name %s", operatorName.Buffer());
                            }
                        }
                        i++;
                    }
