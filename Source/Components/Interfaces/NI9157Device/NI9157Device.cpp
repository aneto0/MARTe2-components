...
NI9157Device::~NI9157Device() {
    ...
    if (isOpened > 0u) {
        status = Reset();
        REPORT_ERROR_PARAMETERS(ErrorManagement::Information, "NI9157Device::~NI9157Device RIO Device resetted with status %d", static_cast<int32>(status));
        status = Close();
...
bool NI9157Device::Initialise(StructuredDataI &data) {
        ...
        if (!ret) {
            ...
            if (ret) {
                ...
                while ((!found) && (ret)) {
                    ...
                    uint32 size = 128u;
                    status = NiFpgaEx_GetAttributeString(candidateDev.Buffer(), NiFpgaEx_AttributeString_SerialNumber, &serialNumber[0u],
                    ...
                    if (ret) {
                        ...
                        REPORT_ERROR(ErrorManagement::InitialisationError, "NI9157Device::Initialise Device with SN = %s not found",
                                     niRioSerialNumber.Buffer());
                    }
                    devN++;
                }
            }
        ...
        if (ret) {
            ...
            if (!ret) {
                REPORT_ERROR(ErrorManagement::InitialisationError,
                             "NI9157Device::Initialise Please specify the NI-RIO generated signature (NiRioGenSignature)");
            }
        }
        if (ret) {
            if (!data.Read("Open", open)) {
                open = 0u;
            }
        }
        if (ret) {
    ...
    if (ret) {
        if (open > 0u) {
            if (clear == 1u) {
                REPORT_ERROR_PARAMETERS(ErrorManagement::Information, "NI9157Device::Initialise - Calling clear fpga method on device %s.",
                ...
                REPORT_ERROR_PARAMETERS((status == 0) ? ErrorManagement::Information : ErrorManagement::Warning,
                                        "NI9157Device::Initialise - Clear fpga method on device %s returning with status %d.", niRioDeviceName.Buffer(),
                                        static_cast<int32>(status));
                status = 0;
                Sleep::MSec(100u);
            }
            REPORT_ERROR_PARAMETERS(ErrorManagement::Information, "NI9157Device::Initialise Opening %s %s %s", niRioDeviceName.Buffer(),
                                    niRioGenSignature.Buffer(), niRioGenFile.Buffer());
            /*lint -e{930} allow conversion from enum to int*/
            status = NiFpga_Open(niRioGenFile.Buffer(), niRioGenSignature.Buffer(), niRioDeviceName.Buffer(), static_cast<uint32_t>(NiFpga_OpenAttribute_NoRun),
                                 &session);
            ret = (status == 0);
            if (ret) {
                isOpened = 1u;
                if (reset == 1u) {
                    ...
                    ret = (status == 0);
                    REPORT_ERROR_PARAMETERS(ret ? ErrorManagement::Information : ErrorManagement::FatalError, "NI9157Device::Initialise Reset() with status=%d",
                                            static_cast<int32>(status));
                    if (ret) {
                        if (resetPostSleepMs != 0u) {
                            REPORT_ERROR_PARAMETERS(ErrorManagement::Information, "NI9157Device::Initialise - Sleeping for %u ms after NiFpga_Reset().",
                                                    resetPostSleepMs);
            ...
            if (data.MoveRelative("Configuration")) {
                ...
                NiFpgaEx_ResourceType resTypes[] = { NiFpgaEx_ResourceType_ControlBool, NiFpgaEx_ResourceType_ControlU8, NiFpgaEx_ResourceType_ControlI8,
                        NiFpgaEx_ResourceType_ControlU16, NiFpgaEx_ResourceType_ControlI16, NiFpgaEx_ResourceType_ControlU32, NiFpgaEx_ResourceType_ControlI32,
                        NiFpgaEx_ResourceType_ControlU64, NiFpgaEx_ResourceType_ControlI64 };
                ...
                for (uint32 k = 0u; (k < numberOfElements) && (ret); k++) {
                    ...
                    while ((suffixes[i] != NULL_PTR(const char8*)) && (!found) && (ret)) {
                        ...
                        if (found) {
                            ...
                            REPORT_ERROR_PARAMETERS(ErrorManagement::Information, "NI9157Device::Initialise Trying to config %s %s", operatorName.Buffer(),
                                                    varName);
                            ...
                            if (ret) {
                                ...
                                if (ret) {
                                    ...
                                    if (ret) {
                                        ...
                                        if (ret) {
                                            ...
                                            if (!ret) {
                                                REPORT_ERROR_PARAMETERS(ErrorManagement::InitialisationError,
                                                                        "NI9157Device::Initialise Failed writing %s=%d status=%d", variable.Buffer(), value,
                                                                        static_cast<int32>(status));
                                            }
                                            else {
                                                REPORT_ERROR_PARAMETERS(ErrorManagement::Information, "NI9157Device::Initialise Variable %s=%d status=%d",
                                        ...
                                        REPORT_ERROR_PARAMETERS(ErrorManagement::InitialisationError,
                                                                "NI9157Device::Initialise NI variable %s not found status=%d", variable.Buffer(),
                                                                static_cast<int32>(status));
                                    }
                                }
                                else {
                                    REPORT_ERROR(ErrorManagement::InitialisationError,
                                                 "NI9157Device::Initialise Failed the creation of NI9157DeviceOperatorTI");
                                }
                            }
                            else {
                                REPORT_ERROR(ErrorManagement::InitialisationError,
                                             "NI9157Device::Initialise Failed the creation of CreateNI9157DeviceOperatorI from name %s", operatorName.Buffer());
                            }
                        }
                        i++;
                    }
                    if (ret) {
                        ...
                        if (!ret) {
                            REPORT_ERROR_PARAMETERS(ErrorManagement::InitialisationError,
                                                    "NI9157Device::Initialise Cannot retrieve the type from the variable name %s", variable.Buffer());
                        }
                    }
                }
                if (configurator != NULL_PTR(NI9157DeviceOperatorTI*)) {
            ...
            if ((isRunning == 0u) && (run > 0u)) {
                ...
                if (status == 0) {
                    ...
                    REPORT_ERROR_PARAMETERS(ErrorManagement::InitialisationError, "NI9157Device::Initialise Failed Run() with status=%d",
                                            static_cast<int32>(status));
                    status = NiFpga_Reset(session);
    ...
    REPORT_ERROR_PARAMETERS(ErrorManagement::Information, "NI9157Device::Initialise returning %s with NI session=%d and status=%d", ret ? "true" : "false",
                            static_cast<int32>(session), static_cast<int32>(status));
    return ret;
}
...
NiFpga_Status NI9157Device::FindResource(const char8 *const varName,
                                         const bool type,
...
NiFpga_Status NI9157Device::NiRead(const uint32 indicator,
                                   bool &value) {
    status = NiFpga_ReadBool(session, static_cast<uint32_t>(indicator), reinterpret_cast<NiFpga_Bool*>(&value));
    return status;
}

NiFpga_Status NI9157Device::NiRead(const uint32 indicator,
                                   int8 &value) {
    status = NiFpga_ReadI8(session, static_cast<uint32_t>(indicator), reinterpret_cast<int8_t*>(&value));
    return status;
}

NiFpga_Status NI9157Device::NiRead(const uint32 indicator,
                                   uint8 &value) {
    status = NiFpga_ReadU8(session, static_cast<uint32_t>(indicator), reinterpret_cast<uint8_t*>(&value));
    return status;
}

NiFpga_Status NI9157Device::NiRead(const uint32 indicator,
                                   int16 &value) {
    status = NiFpga_ReadI16(session, static_cast<uint32_t>(indicator), reinterpret_cast<int16_t*>(&value));
    return status;
}

NiFpga_Status NI9157Device::NiRead(const uint32 indicator,
                                   uint16 &value) {
    status = NiFpga_ReadU16(session, static_cast<uint32_t>(indicator), reinterpret_cast<uint16_t*>(&value));
    return status;
}

NiFpga_Status NI9157Device::NiRead(const uint32 indicator,
                                   int32 &value) {
    status = NiFpga_ReadI32(session, static_cast<uint32_t>(indicator), reinterpret_cast<int32_t*>(&value));
    return status;
}

NiFpga_Status NI9157Device::NiRead(const uint32 indicator,
                                   uint32 &value) {
    status = NiFpga_ReadU32(session, static_cast<uint32_t>(indicator), reinterpret_cast<uint32_t*>(&value));
    return status;
}

NiFpga_Status NI9157Device::NiRead(const uint32 indicator,
                                   int64 &value) {
    status = NiFpga_ReadI64(session, static_cast<uint32_t>(indicator), reinterpret_cast<int64_t*>(&value));
    return status;
}

NiFpga_Status NI9157Device::NiRead(const uint32 indicator,
                                   uint64 &value) {
    status = NiFpga_ReadU64(session, static_cast<uint32_t>(indicator), reinterpret_cast<uint64_t*>(&value));
    return status;
}

NiFpga_Status NI9157Device::NiWrite(const uint32 control,
                                    const bool value) {
    status = NiFpga_WriteBool(session, static_cast<uint32_t>(control), static_cast<NiFpga_Bool>(value));
    return status;
}

NiFpga_Status NI9157Device::NiWrite(const uint32 control,
                                    const int8 value) {
    status = NiFpga_WriteI8(session, static_cast<uint32_t>(control), static_cast<int8_t>(value));
    return status;
}

NiFpga_Status NI9157Device::NiWrite(const uint32 control,
                                    const uint8 value) {
    status = NiFpga_WriteU8(session, static_cast<uint32_t>(control), static_cast<uint8_t>(value));
    return status;
}

NiFpga_Status NI9157Device::NiWrite(const uint32 control,
                                    const int16 value) {
    status = NiFpga_WriteI16(session, static_cast<uint32_t>(control), static_cast<int16_t>(value));
    return status;
}

NiFpga_Status NI9157Device::NiWrite(const uint32 control,
                                    const uint16 value) {
    status = NiFpga_WriteU16(session, static_cast<uint32_t>(control), static_cast<uint16_t>(value));
    return status;
}

NiFpga_Status NI9157Device::NiWrite(const uint32 control,
                                    const int32 value) {
    status = NiFpga_WriteI32(session, static_cast<uint32_t>(control), static_cast<int32_t>(value));
    return status;
}

NiFpga_Status NI9157Device::NiWrite(const uint32 control,
                                    const uint32 value) {
    status = NiFpga_WriteU32(session, static_cast<uint32_t>(control), static_cast<uint32_t>(value));
    return status;
}

NiFpga_Status NI9157Device::NiWrite(const uint32 control,
                                    const int64 value) {
    status = NiFpga_WriteI64(session, static_cast<uint32_t>(control), static_cast<int64_t>(value));
    return status;
}

NiFpga_Status NI9157Device::NiWrite(const uint32 control,
                                    const uint64 value) {
    status = NiFpga_WriteU64(session, static_cast<uint32_t>(control), static_cast<uint64_t>(value));
    return status;
}
...
NiFpga_Status NI9157Device::NiConfigureFifo(const uint32 fifo,
    ...
    status = NiFpga_ConfigureFifo2(session, niFifo, requestedDepthST, &actualDepthST);
    actualDepth = static_cast<uint32>(actualDepthST);
    return status;
}

NiFpga_Status NI9157Device::NiStartFifo(const uint32 fifo) {
    status = NiFpga_StartFifo(session, static_cast<uint32_t>(fifo));
    return status;
}

NiFpga_Status NI9157Device::NiStopFifo(const uint32 fifo) {
    status=NiFpga_StopFifo(session, static_cast<uint32_t>(fifo));
    return status;
}

NiFpga_Status NI9157Device::NiReadFifo(const uint32 fifo,
                                       bool *const data,
    ...
    status = NiFpga_ReadFifoBool(session, niFifo, reinterpret_cast<NiFpga_Bool*>(data), numberOfElementsST, static_cast<uint32_t>(timeout),
                                      &elementsRemainingST);
...
NiFpga_Status NI9157Device::NiReadFifo(const uint32 fifo,
    ...
    size_t numberOfElementsST = static_cast<size_t>(numberOfElements);
    size_t elementsRemainingST = static_cast<size_t>(0u);
    status = NiFpga_ReadFifoI8(session, niFifo, reinterpret_cast<int8_t*>(data), numberOfElementsST, static_cast<uint32_t>(timeout), &elementsRemainingST);
    elementsRemaining = static_cast<uint32>(elementsRemainingST);
    return status;
}
...
NiFpga_Status NI9157Device::NiReadFifo(const uint32 fifo,
    ...
    size_t numberOfElementsST = static_cast<size_t>(numberOfElements);
    size_t elementsRemainingST = static_cast<size_t>(0u);
    status = NiFpga_ReadFifoU8(session, niFifo, reinterpret_cast<uint8_t*>(data), numberOfElementsST, static_cast<uint32_t>(timeout),
                                    &elementsRemainingST);
    elementsRemaining = static_cast<uint32>(elementsRemainingST);
    return status;
}
...
NiFpga_Status NI9157Device::NiReadFifo(const uint32 fifo,
    ...
    size_t numberOfElementsST = static_cast<size_t>(numberOfElements);
    size_t elementsRemainingST = static_cast<size_t>(0u);
    status = NiFpga_ReadFifoI16(session, niFifo, reinterpret_cast<int16_t*>(data), numberOfElementsST, static_cast<uint32_t>(timeout),
                                     &elementsRemainingST);
    elementsRemaining = static_cast<uint32>(elementsRemainingST);
    return status;
}
...
NiFpga_Status NI9157Device::NiReadFifo(const uint32 fifo,
    ...
    size_t numberOfElementsST = static_cast<size_t>(numberOfElements);
    size_t elementsRemainingST = static_cast<size_t>(0u);
    status = NiFpga_ReadFifoU16(session, niFifo, reinterpret_cast<uint16_t*>(data), numberOfElementsST, static_cast<uint32_t>(timeout),
                                     &elementsRemainingST);
    elementsRemaining = static_cast<uint32>(elementsRemainingST);
    return status;
}
...
NiFpga_Status NI9157Device::NiReadFifo(const uint32 fifo,
    ...
    size_t numberOfElementsST = static_cast<size_t>(numberOfElements);
    size_t elementsRemainingST = static_cast<size_t>(0u);
    status = NiFpga_ReadFifoI32(session, niFifo, reinterpret_cast<int32_t*>(data), numberOfElementsST, static_cast<uint32_t>(timeout),
                                     &elementsRemainingST);
    elementsRemaining = static_cast<uint32>(elementsRemainingST);
    return status;
}
...
NiFpga_Status NI9157Device::NiReadFifo(const uint32 fifo,
    ...
    size_t numberOfElementsST = static_cast<size_t>(numberOfElements);
    size_t elementsRemainingST = static_cast<size_t>(0u);
    status = NiFpga_ReadFifoU32(session, niFifo, reinterpret_cast<uint32_t*>(data), numberOfElementsST, static_cast<uint32_t>(timeout),
                                     &elementsRemainingST);
    elementsRemaining = static_cast<uint32>(elementsRemainingST);
    return status;
}
...
NiFpga_Status NI9157Device::NiReadFifo(const uint32 fifo,
    ...
    size_t numberOfElementsST = static_cast<size_t>(numberOfElements);
    size_t elementsRemainingST = static_cast<size_t>(0u);
    status = NiFpga_ReadFifoI64(session, niFifo, reinterpret_cast<int64_t*>(data), numberOfElementsST, static_cast<uint32_t>(timeout),
                                     &elementsRemainingST);
    elementsRemaining = static_cast<uint32>(elementsRemainingST);
    return status;
}
...
NiFpga_Status NI9157Device::NiReadFifo(const uint32 fifo,
    ...
    size_t numberOfElementsST = static_cast<size_t>(numberOfElements);
    size_t elementsRemainingST = static_cast<size_t>(0u);
    status = NiFpga_ReadFifoU64(session, niFifo, reinterpret_cast<uint64_t*>(data), numberOfElementsST, static_cast<uint32_t>(timeout),
                                     &elementsRemainingST);
    elementsRemaining = static_cast<uint32>(elementsRemainingST);
    return status;
}

NiFpga_Status NI9157Device::NiWriteFifo(const uint32 fifo,
    ...
    status = NiFpga_WriteFifoBool(session, niFifo, reinterpret_cast<const NiFpga_Bool*>(data), numberOfElementsST, static_cast<uint32_t>(timeout),
                                       &emptyElementsRemainingST);
...
NiFpga_Status NI9157Device::NiWriteFifo(const uint32 fifo,
    ...
    size_t numberOfElementsST = static_cast<size_t>(numberOfElements);
    size_t emptyElementsRemainingST = static_cast<size_t>(0u);
    status = NiFpga_WriteFifoI8(session, niFifo, reinterpret_cast<const int8_t*>(data), numberOfElementsST, static_cast<uint32_t>(timeout),
                                     &emptyElementsRemainingST);
    emptyElementsRemaining = static_cast<uint32>(emptyElementsRemainingST);
    return status;
}
...
NiFpga_Status NI9157Device::NiWriteFifo(const uint32 fifo,
    ...
    size_t numberOfElementsST = static_cast<size_t>(numberOfElements);
    size_t emptyElementsRemainingST = static_cast<size_t>(0u);
    status = NiFpga_WriteFifoU8(session, niFifo, reinterpret_cast<const uint8_t*>(data), numberOfElementsST, static_cast<uint32_t>(timeout),
                                     &emptyElementsRemainingST);
    emptyElementsRemaining = static_cast<uint32>(emptyElementsRemainingST);
    return status;
}
...
NiFpga_Status NI9157Device::NiWriteFifo(const uint32 fifo,
    ...
    size_t numberOfElementsST = static_cast<size_t>(numberOfElements);
    size_t emptyElementsRemainingST = static_cast<size_t>(0u);
    status = NiFpga_WriteFifoI16(session, niFifo, reinterpret_cast<const int16_t*>(data), numberOfElementsST, static_cast<uint32_t>(timeout),
                                      &emptyElementsRemainingST);
    emptyElementsRemaining = static_cast<uint32>(emptyElementsRemainingST);
    return status;
}
...
NiFpga_Status NI9157Device::NiWriteFifo(const uint32 fifo,
    ...
    size_t numberOfElementsST = static_cast<size_t>(numberOfElements);
    size_t emptyElementsRemainingST = static_cast<size_t>(0u);
    status = NiFpga_WriteFifoU16(session, niFifo, reinterpret_cast<const uint16_t*>(data), numberOfElementsST, static_cast<uint32_t>(timeout),
                                      &emptyElementsRemainingST);
    emptyElementsRemaining = static_cast<uint32>(emptyElementsRemainingST);
    return status;
}
...
NiFpga_Status NI9157Device::NiWriteFifo(const uint32 fifo,
    ...
    size_t numberOfElementsST = static_cast<size_t>(numberOfElements);
    size_t emptyElementsRemainingST = static_cast<size_t>(0u);
    status = NiFpga_WriteFifoI32(session, niFifo, reinterpret_cast<const int32_t*>(data), numberOfElementsST, static_cast<uint32_t>(timeout),
                                      &emptyElementsRemainingST);
    emptyElementsRemaining = static_cast<uint32>(emptyElementsRemainingST);
    return status;
}
...
NiFpga_Status NI9157Device::NiWriteFifo(const uint32 fifo,
    ...
    size_t numberOfElementsST = static_cast<size_t>(numberOfElements);
    size_t emptyElementsRemainingST = static_cast<size_t>(0u);
    status = NiFpga_WriteFifoU32(session, niFifo, reinterpret_cast<const uint32_t*>(data), numberOfElementsST, static_cast<uint32_t>(timeout),
                                      &emptyElementsRemainingST);
    emptyElementsRemaining = static_cast<uint32>(emptyElementsRemainingST);
    return status;
}
...
NiFpga_Status NI9157Device::NiWriteFifo(const uint32 fifo,
    ...
    size_t numberOfElementsST = static_cast<size_t>(numberOfElements);
    size_t emptyElementsRemainingST = static_cast<size_t>(0u);
    status = NiFpga_WriteFifoI64(session, niFifo, reinterpret_cast<const int64_t*>(data), numberOfElementsST, static_cast<uint32_t>(timeout),
                                      &emptyElementsRemainingST);
    emptyElementsRemaining = static_cast<uint32>(emptyElementsRemainingST);
    return status;
}
...
NiFpga_Status NI9157Device::NiWriteFifo(const uint32 fifo,
    ...
    size_t numberOfElementsST = static_cast<size_t>(numberOfElements);
    size_t emptyElementsRemainingST = static_cast<size_t>(0u);
    status = NiFpga_WriteFifoU64(session, niFifo, reinterpret_cast<const uint64_t*>(data), numberOfElementsST, static_cast<uint32_t>(timeout),
                                      &emptyElementsRemainingST);
    emptyElementsRemaining = static_cast<uint32>(emptyElementsRemainingST);
    return status;
}
...
ErrorManagement::ErrorType NI9157Device::WriteParam(StreamString varName,
                                                    const uint64 value,
                                                    StreamString type) {
    ...
    if (type == "bool") {
        ...
        if (err.ErrorsCleared()) {
            err = (NiWrite(varId, static_cast<bool>(value)) != 0);
        }
    }
    else if (type == "uint8") {
        uint32 varId;
        ...
        err = (FindResource(varName.Buffer(), rType, varId) != 0);
        if (err.ErrorsCleared()) {
            err = (NiWrite(varId, static_cast<uint8>(value)) != 0);
        }
    }
    else if (type == "uint16") {
        uint32 varId;
        ...
        err = (FindResource(varName.Buffer(), rType, varId) != 0);
        if (err.ErrorsCleared()) {
            err = (NiWrite(varId, static_cast<uint16>(value)) != 0);
        }
    }
    else if (type == "uint32") {
        uint32 varId;
        ...
        err = (FindResource(varName.Buffer(), rType, varId) != 0);
        if (err.ErrorsCleared()) {
            err = (NiWrite(varId, static_cast<uint32>(value)) != 0);
        }
    }
    else if (type == "uint64") {
        uint32 varId;
        ...
        err = (FindResource(varName.Buffer(), rType, varId) != 0);
        if (err.ErrorsCleared()) {
            err = (NiWrite(varId, static_cast<uint64>(value)) != 0);
        }
    }
    else if (type == "int8") {
        uint32 varId;
        ...
        err = (FindResource(varName.Buffer(), rType, varId) != 0);
        if (err.ErrorsCleared()) {
            err = (NiWrite(varId, static_cast<int8>(value)) != 0);
        }
    }
    else if (type == "int16") {
        uint32 varId;
        ...
        err = (FindResource(varName.Buffer(), rType, varId) != 0);
        if (err.ErrorsCleared()) {
            err = (NiWrite(varId, static_cast<int16>(value)) != 0);
        }
    }
    else if (type == "int32") {
        uint32 varId;
        ...
        err = (FindResource(varName.Buffer(), rType, varId) != 0);
        if (err.ErrorsCleared()) {
            err = (NiWrite(varId, static_cast<int32>(value)) != 0);
        }
    }
    else if (type == "int64") {
        uint32 varId;
        ...
        err = (FindResource(varName.Buffer(), rType, varId) != 0);
        if (err.ErrorsCleared()) {
            err = (NiWrite(varId, static_cast<int64>(value)) != 0);
        }
    }
    else {
        err.SetError(4u);
...
ErrorManagement::ErrorType NI9157Device::ReadParam(StreamString varName,
                                                   uint64 &value,
                                                   StreamString type) {
    ErrorManagement::ErrorType err;
    if (type == "bool") {
        ...
        err = (FindResource(varName.Buffer(), rType, varId) != 0);
        if (err.ErrorsCleared()) {
            err = (NiRead(varId, *(reinterpret_cast<bool*>(&value))) != 0);
        }
    }
    else if (type == "uint8") {
        uint32 varId;
        ...
        err = (FindResource(varName.Buffer(), rType, varId) != 0);
        if (err.ErrorsCleared()) {
            err = (NiRead(varId, *(reinterpret_cast<uint8*>(&value))) != 0);
        }
    }
    else if (type == "uint16") {
        uint32 varId;
        ...
        err = (FindResource(varName.Buffer(), rType, varId) != 0);
        if (err.ErrorsCleared()) {
            err = (NiRead(varId, *(reinterpret_cast<uint16*>(&value))) != 0);
        }
    }
    else if (type == "uint32") {
        uint32 varId;
        ...
        err = (FindResource(varName.Buffer(), rType, varId) != 0);
        if (err.ErrorsCleared()) {
            err = (NiRead(varId, *(reinterpret_cast<uint32*>(&value))) != 0);
        }
    }
    else if (type == "uint64") {
        uint32 varId;
        ...
        err = (FindResource(varName.Buffer(), rType, varId) != 0);
        if (err.ErrorsCleared()) {
            err = (NiRead(varId, *(reinterpret_cast<uint64*>(&value))) != 0);
        }
    }
    else if (type == "int8") {
        uint32 varId;
        ...
        err = (FindResource(varName.Buffer(), rType, varId) != 0);
        if (err.ErrorsCleared()) {
            err = (NiRead(varId, *(reinterpret_cast<int8*>(&value))) != 0);
        }
    }
    else if (type == "int16") {
        uint32 varId;
        ...
        err = (FindResource(varName.Buffer(), rType, varId) != 0);
        if (err.ErrorsCleared()) {
            err = (NiRead(varId, *(reinterpret_cast<int16*>(&value))) != 0);
        }
    }
    else if (type == "int32") {
        uint32 varId;
        ...
        err = (FindResource(varName.Buffer(), rType, varId) != 0);
        if (err.ErrorsCleared()) {
            err = (NiRead(varId, *(reinterpret_cast<int32*>(&value))) != 0);
        }
    }
    else if (type == "int64") {
        uint32 varId;
        ...
        err = (FindResource(varName.Buffer(), rType, varId) != 0);
        if (err.ErrorsCleared()) {
            err = (NiRead(varId, *(reinterpret_cast<int64*>(&value))) != 0);
        }
    }
    else {
        err.SetError(4u);
