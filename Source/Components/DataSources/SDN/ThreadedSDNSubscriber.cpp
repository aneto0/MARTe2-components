/**
 * @file ThreadedSDNSubscriber.cpp
 * @brief Source file for class ThreadedSDNSubscriber
 * @date 20/12/2016
...
ThreadedSDNSubscriber::ThreadedSDNSubscriber() :
        DataSourceI(),
        EmbeddedServiceMethodBinderI(),
        executor(*this) {

    log_trace("ThreadedSDNSubscriber::ThreadedSDNSubscriber - Entering method");
    ...
    log_trace("ThreadedSDNSubscriber::ThreadedSDNSubscriber - Leaving method");
...
bool ThreadedSDNSubscriber::Initialise(StructuredDataI &data) {

    log_trace("ThreadedSDNSubscriber::Initialise - Entering method");

    bool ok = DataSourceI::Initialise(data);

    if (!ok) {
        log_error("ThreadedSDNSubscriber::SetConfiguredDatabase - Parent virtual method failed");
    ...
    if (!data.Read("Interface", ifaceName)) {
        log_error("ThreadedSDNSubscriber::Initialise - Interface attribute must be specified");
        REPORT_ERROR(ErrorManagement::ParametersError, "Interface must be specified");
	ok = false;
    } else {
        log_info("ThreadedSDNSubscriber::Initialise - Interface is '%s'", ifaceName.Buffer());
    } 

    if (!net_is_interface_valid(ifaceName.Buffer())) {
        log_error("ThreadedSDNSubscriber::Initialise - Interface '%s' is not valid", ifaceName.Buffer());
        REPORT_ERROR(ErrorManagement::ParametersError, "Interface must be a valid identifier");
	ok = false;
    } else {
        log_info("ThreadedSDNSubscriber::Initialise - Interface '%s' is valid", ifaceName.Buffer());
    } 

    // Retrieve and verify topic name
    if (!data.Read("Topic", topicName)) {
        log_error("ThreadedSDNSubscriber::Initialise - Topic must be specified");
        REPORT_ERROR(ErrorManagement::ParametersError, "Topic must be specified");
	ok = false;
    } else {
        log_info("ThreadedSDNSubscriber::Initialise - Topic name is '%s'", topicName.Buffer());
    }
    ...
    if (data.Read("Address", destAddr)) {
        ...
        log_info("ThreadedSDNSubscriber::Initialise - Explicit destination address '%s'", destAddr.Buffer());

	if (!sdn_is_address_valid(destAddr.Buffer())) {
	    log_error("ThreadedSDNSubscriber::Initialise - Address '%s' is not valid", destAddr.Buffer());
	    REPORT_ERROR(ErrorManagement::ParametersError, "Address must be a valid identifier, i.e. '<IP_addr>:<port>'");
	    ok = false;
	} else {
	    log_info("ThreadedSDNSubscriber::Initialise - Valid destination address '%s'", destAddr.Buffer());
	}
    ...
    if (data.Read("Timeout", timeout)) {
        log_info("ThreadedSDNSubscriber::Initialise - Explicit timeout '%u'", timeout);
	TTTimeout = timeout;
    }

    // Synchronising mode parameter
    if (data.Read("Mode", modeName)) {
        log_info("ThreadedSDNSubscriber::Initialise - Explicit synchronisation mode '%s'", modeName.Buffer());
	if ((modeName == "Default") || (modeName == "Caching")) {
	    mode = Default;
	} else if (modeName == "Synchronising") {
	    mode = Synchronising;
	} else {
	    log_error("ThreadedSDNSubscriber::Initialise - Mode '%s' is not valid", modeName.Buffer());
	    REPORT_ERROR(ErrorManagement::ParametersError, "Mode must be a valid identifier, i.e. Default|Caching|Synchronising");
	    ok = false;
	}
    }

    log_trace("ThreadedSDNSubscriber::Initialise - Leaving method");

    return ok;
}

bool ThreadedSDNSubscriber::SetConfiguredDatabase(StructuredDataI& data) {

    log_trace("ThreadedSDNSubscriber::SetConfiguredDatabase - Entering method");
    ...
    if (!ok) {
        log_error("ThreadedSDNSubscriber::SetConfiguredDatabase - nOfSignals must be > 0u");
        REPORT_ERROR(ErrorManagement::ParametersError, "nOfSignals must be > 0u");
    } else {
        log_info("ThreadedSDNSubscriber::SetConfiguredDatabase - Number of signals '%u'", nOfSignals);
    }
      
    log_trace("ThreadedSDNSubscriber::SetConfiguredDatabase - Leaving method");

    return ok;
}

bool ThreadedSDNSubscriber::AllocateMemory() {

    log_trace("ThreadedSDNSubscriber::AllocateMemory - Entering method");
	...
	if (!ok) {
	  log_warning("ThreadedSDNSubscriber::AllocateMemory - Unable to query signal '%u' name", signalIndex);
    ...
    log_trace("ThreadedSDNSubscriber::AllocateMemory - Leaving method");

    return ok;
}

uint32 ThreadedSDNSubscriber::GetNumberOfMemoryBuffers() {
    return 1u;
}

bool ThreadedSDNSubscriber::GetSignalMemoryBuffer(const uint32 signalIdx,
						  const uint32 bufferIdx,
						  void*& signalAddress) {

    log_trace("ThreadedSDNSubscriber::GetSignalMemoryBuffer - Entering method");
    ...
    if (ok) {
        ...
        log_info("ThreadedSDNSubscriber::GetSignalMemoryBuffer - Reference of signal '%u' if '%p'", signalIdx, signalAddress);
    }

    log_trace("ThreadedSDNSubscriber::GetSignalMemoryBuffer - Leaving method");

    return ok;
}

const char8* ThreadedSDNSubscriber::GetBrokerName(StructuredDataI& data,
...
bool ThreadedSDNSubscriber::GetInputBrokers(ReferenceContainer& inputBrokers,
					    const char8* const functionName,
					    void* const gamMemPtr) {

    log_trace("ThreadedSDNSubscriber::GetInputBrokers - Entering method");
    ...
    log_trace("ThreadedSDNSubscriber::GetInputBrokers - Leaving method");

    return ok;
}

/*lint -e{715}  [MISRA C++ Rule 0-1-11], [MISRA C++ Rule 0-1-12]. Justification: returns false irrespectively of the input parameters.*/
bool ThreadedSDNSubscriber::GetOutputBrokers(ReferenceContainer& outputBrokers,
					     const char8* const functionName,
					     void* const gamMemPtr) {
    return false;
}
...
bool ThreadedSDNSubscriber::PrepareNextState(const char8* const currentStateName,
					     const char8* const nextStateName) {

    log_trace("ThreadedSDNSubscriber::PrepareNextState - Entering method");

    bool ok = (subscriber != NULL_PTR(sdn::Subscriber *));

    if (!ok) {
        log_error("ThreadedSDNSubscriber::PrepareNextState - sdn::Subscriber has not been initiaised");
    ...
    log_trace("ThreadedSDNSubscriber::PrepareNextState - Leaving method");

    return ok;
}

bool ThreadedSDNSubscriber::Synchronise() {

    log_trace("ThreadedSDNSubscriber::Synchronise - Entering method");

    bool ok = (mode == Synchronising); // Synchronising RT thread

    if (!ok) {
        log_error("ThreadedSDNSubscriber::Synchronise - Invalid method call for non-synchronising DataSourceI");
        REPORT_ERROR(ErrorManagement::FatalError, "ThreadedSDNSubscriber operates in Default (caching) mode");
    }
    ...
    log_trace("ThreadedSDNSubscriber::Synchronise - Leaving method");

    return ok;
}

/*lint -e{715}  [MISRA C++ Rule 0-1-11], [MISRA C++ Rule 0-1-12]. Justification: the method operates regardles of the input parameter.*/
ErrorManagement::ErrorType ThreadedSDNSubscriber::Execute(const ExecutionInfo& info) {
...
CLASS_REGISTER(ThreadedSDNSubscriber, "1.0.10")
