...
bool UARTDataSource::Initialise(StructuredDataI &data) {
    ...
    if (ok) {
        if (!data.Read("TimeoutToSynchronise", timeoutToSynchronise)) {
            REPORT_ERROR(ErrorManagement::FatalError, "TimeoutToSynchronise not specified: set to %d", timeoutToSynchronise);
        }
    }
...
ErrorManagement::ErrorType UARTDataSource::CRIOThreadCallback(ExecutionInfo &info) {
    ...
    if (info.GetStage() == ExecutionInfo::MainStage) {
        ...
        uint32 serialWriteIdx = (numberOfBuffers * (sizeof(uint64) + sizeof(uint8))) + (lastWrittenIdx * packetByteSize);
        uint32 timeWriteIdx = (numberOfBuffers) + (lastWrittenIdx * sizeof(uint64));
        uint32 timeWriteIdx_1 = numberOfBuffers;
        if (lastWrittenIdx > 0u) {
            timeWriteIdx_1 += ((lastWrittenIdx - 1u) * sizeof(uint64));
        }
        else {
            timeWriteIdx_1 += ((numberOfBuffers - 1u) * sizeof(uint64));
        }
        ...
        *timePacket = *reinterpret_cast<uint64 *>(&(memory[timeWriteIdx_1]));
        ...
        if (serial.Read(reinterpret_cast<char8 *>(&(memory[serialWriteIdx])), bytesToRead, serialTimeout)) {
            ...
            err = (tcn_get_time((hpn_timestamp_t*) (timePacket)) != TCN_SUCCESS);
            muxSem.FastUnLock();
        }
        else {
            ...
            //Read only part of the packet from the serial.Read.
                ...
                REPORT_ERROR(ErrorManagement::Warning, "Failed to read %d bytes from serial. Trying to resynchronise by waiting %d us for no data",
                             packetByteSize, timeoutToSynchronise);
                while (serial.WaitRead(timeoutToSynchronise)) {
                    char8 ignoredMem;
                    uint32 ignoredSize = 1u;
                    (void) serial.Read(&ignoredMem, ignoredSize);
                }
                REPORT_ERROR(ErrorManagement::Warning, "No data arrived in the last %d us. As such, the next packet should be synchronised.",
                             timeoutToSynchronise);
            }
        }
    }
...
bool UARTDataSource::GetInputOffset(const uint32 signalIdx,
    ...
    if (signalIdx == 0u) {
        ...
        offset = (lastReadIdx * sizeof(uint64));
    }
    else {
